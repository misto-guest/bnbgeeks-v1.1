...
async function main() {
  ...
  try {
    ...
    const portalsRes = await axios.get(`${API_URL}/portals`);
    const portals: Portal[] = portalsRes.data.portals;
    ...
    const batchRes = await axios.post(`${API_URL}/search/batch`, {
      queries,
    ...
    while (attempts < maxAttempts) {
      ...
      const statsRes = await axios.get(`${API_URL}/results/stats`);
    ...
    const resultsRes = await axios.get(`${API_URL}/results/confirmed`);
    const results: SearchResult[] = resultsRes.data.results;
    ...
    if (results.length > 0) {
      ...
      // JSON export
      const exportRes = await axios.get(`${API_URL}/results/export`, {
        responseType: 'arraybuffer'
      });

      const exportPath = path.join(__dirname, '../data/brand-mentions.json');
      fs.writeFileSync(exportPath, exportRes.data);
      console.log(`  ✓ JSON exported to: ${exportPath}`);

      // Generate report
      const reportPath = path.join(__dirname, '../RESULTS_REPORT.md');
...
function generateReport(results: SearchResult[], outputPath: string) {
