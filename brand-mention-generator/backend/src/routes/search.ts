...
import searchAutomation from '../utils/searchAutomation';
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    // Find portal by URL
    const portals = PortalsModel.getAllPortals();
    ...
    // Perform search
    const result = await searchAutomation.searchPortal(portal, query);
...
router.post('/batch', async (req: Request, res: Response) => {
  try {
    ...
    let portals = PortalsModel.getActivePortals();
    ...
    setImmediate(async () => {
      try {
        const results = await searchAutomation.batchSearch(portals, queries);
        ResultsModel.addResults(results);
